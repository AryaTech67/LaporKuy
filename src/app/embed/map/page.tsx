    ...
    <div className="w-full h-screen relative bg-slate-950 text-slate-100 flex flex-col font-sans overflow-hidden">
      ...
      <div className="bg-slate-900/95 backdrop-blur-md px-4 py-2.5 border-b border-slate-800 flex items-center justify-between z-20 shrink-0">
        <div className="flex items-center gap-3">
          <Link href={isAdmin ? "/admin" : "/"} className="hover:opacity-90 transition-opacity" title={isAdmin ? "Kembali ke Admin" : "Ke Beranda"}>
        ...
        <div className="flex items-center gap-2.5">
          ...
          {/* Tombol Balik ke Admin (Hanya tampil jika yang akses adalah Admin) */}
          {isAdmin ? (
            <Link
              href="/admin"
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 shadow-sm transition-all"
            >
      ...
      {/* Floating Button on Top of Map (Khusus Admin) */}
      {isAdmin && (
